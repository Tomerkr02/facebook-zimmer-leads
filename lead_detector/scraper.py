...
def scrape_group_posts() -> None:
    ...
    if not settings.facebook_group_url:
        raise ValueError("FACEBOOK_GROUP_URL is required.")
    ...
    with sync_playwright() as playwright:
        ...
        logger.info("Opening Facebook group: %s", settings.facebook_group_url)
        page.goto(settings.facebook_group_url, wait_until="domcontentloaded", timeout=90000)
        ...
        logger.info("Collected %s possible post containers.", len(post_handles))
        ...
        for handle in post_handles:
            try:
                ...
                logger.warning("Failed to extract a post candidate: %s", exc)
                continue
            ...
            if storage.has_seen(candidate.post_key, candidate.post_url, candidate.text):
                logger.info(
                    "DUPLICATE_SKIPPED | key=%s | url=%s | preview=%s",
                    candidate.post_key,
            ...
            if not candidate.match.is_relevant:
                logger.info(
                    "REJECTED_POST | key=%s | score=%s | reason=%s | url=%s | preview=%s",
                    candidate.post_key,
            ...
            if settings.enable_ai_scoring:
                if not settings.openai_api_key:
                    logger.warning(
                        "AI_SCORING_SKIPPED | key=%s | reason=missing_openai_api_key | preview=%s",
                        candidate.post_key,
                        preview,
                    )
                elif not is_text_reasonable_for_ai(candidate.text):
                    logger.info(
                        "AI_SCORING_SKIPPED | key=%s | reason=text_length_not_reasonable | preview=%s",
                        candidate.post_key,
                        preview,
                    )
                else:
                    try:
                        ...
                        logger.info(
                            "AI_SCORE_RESULT | key=%s | ai_score=%s | category=%s | relevant=%s | preview=%s",
                            candidate.post_key,
                        ...
                        logger.warning(
                            "AI_SCORING_FAILED | key=%s | error=%s | preview=%s",
                            candidate.post_key,
                            exc,
                            preview,
                        )

            if settings.enable_ai_scoring and candidate.ai_result:
                if not candidate.ai_result.is_relevant or candidate.ai_result.score < settings.ai_min_score:
                    logger.info(
                        "REJECTED_POST | key=%s | score=%s | reason=ai_rejected | ai_score=%s | category=%s | url=%s | preview=%s",
                        candidate.post_key,
                        candidate.match.score,
                        candidate.ai_result.score,
                        candidate.ai_result.category,
            ...
            relevant += 1
            logger.info(
                "MATCHED_LEAD | key=%s | score=%s | ai_score=%s | keywords=%s | url=%s | preview=%s",
                candidate.post_key,
                candidate.match.score,
            ...
            if send_message(settings.telegram_bot_token, settings.telegram_chat_id, message):
                alerts_sent += 1
                logger.info(
                    "ALERT_SENT | key=%s | score=%s | url=%s | preview=%s",
                    candidate.post_key,
                    candidate.match.score,
                    candidate.post_url or "-",
                    preview,
    ...
    logger.info(
        "Scan complete | scanned=%s | relevant=%s | alerts_sent=%s",
        scanned,
        relevant,
        alerts_sent,
    )
